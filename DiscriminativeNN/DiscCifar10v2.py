...
class _netD(nn.Module):
    def __init__(self, ndf, nc, wasserstein, ac_gan, n_class, bias, dropout, bn_momentum):
        ...
        if ac_gan:
            self.conv5 = nn.Conv2d(ndf * 8, 1 + ndf * 8, 2, 1, 0, bias=bias)
            self.fc1 = nn.Linear(ndf * 8, ndf * 8)
            self.fc2 = nn.Linear(ndf * 8, self.n_class)
        else:
    ...
    def forward(self, input):
        ...
        if not self.ac_gan:
            ...
            input = input.view(-1, 1 + self.ndf * 8)
            return input[:,0], self.fc2(F.relu(self.fc1(F.relu(input[:,1:]))))
